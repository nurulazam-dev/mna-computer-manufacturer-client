import { faStarHalfStroke,faQuoteRight } from '@fortawesome/free-solid-svg-icons';
...
const Review = () => {
    const { data: reviews, isLoading } = useQuery("reviews", () =>
        fetch("https://mna-computer-manufacturer.onrender.com/reviews").then((res) => res.json())
    );
    if (isLoading) {
                ...
                <div>
                    {
                        reviews?.map(review => (
                            <SwiperSlide key={review._id} className='pb-5 px-10 lg:px-4'>
                                <div className="card h-80 max-w-md lg:max-w-lg my-5 bg-base-100 shadow-xl">
                                    <div className="flex justify-between px-5 items-center my-2">
                                        ...
                                        <div>
                                            <FontAwesomeIcon icon={faQuoteRight} className='text-[120px] opacity-10' />
                                        </div>
                                    </div>
                                    <div className='mx-3 pb-3 text-[16px]'>
                                        ...
                                        <hr />
                                        <p className='mx-2 pt-2'>{review.review.slice(0, 125)}<span className="font-bold">...</span> </p>
                                    </div>
                                </div>
